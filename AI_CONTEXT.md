...
- **版本**：v1.2.2（标签未 bump，累计：全面扫描修复批次 + 农历模块同步 + 农历/阳历双模式切换 + 合婚上下布局 + 合婚时段模式修复 + xiYong ratio 细分 + 五行补救文案强/弱区分 + 太极 logo + 输入侧晚子时/节气临界提示语 + 断语库 504→801 + **死规则清理 801→789 + 空亡断语修活**）
...
- **断语库**：`kb/04-rules-db/rules.json` — **789 条**（事业97/性格77/财运57/用神喜忌57/六亲44/流日50/十神组合46/婚姻47/神煞39/流月45/格局41/健康42/学业41/五行生克36/流年30/大运20/合婚20），每条含 `suggestion` 建议字段；2026-08-19 清理 12 条死规则（801→789）+ 修活空亡（`cc5e405`）
- **UI**：`ui/index.html` 单文件离线，**零外部依赖**（无 Google Fonts），墨底 + 古铜金高端商务风，壹~玖中文序号徽章，内置 206 年节气
- **引擎**：`engine/engine.dist.js`（245KB，UMD 双端，含 getRemedy/yongShenChong/农历模块 + 789 条规则数据；v1.2.2 MD5 `FEC924A713D83E38B3BDF81E4A055AA1`）
...
| 断语库 | 789 条（v1.2.2，2026-08-19 清理 12 条死规则 801→789），路线图 434→504→801→789→1000；每条必须含 `suggestion` 建议字段；规则命中须可追溯古籍出处 |
...
2. **[协作] bazi-app C 端**：引擎已同步（v1.2.2，789 条规则，MD5 `FEC924A713D83E38B3BDF81E4A055AA1`），C 端部署/虎皮椒开通在另一个对话推进（读 `docs/ENGINE-CHANGES.md` 对齐）
3. ✅ ~~断语库扩充 504→800+~~（已完成 `bd7f5ab`，801 条，2026-08-17）
4. ✅ ~~晚子时/节气临界**输入侧**提示语~~（已完成 `53a3bd2`，2026-08-17）
5. **[后置] 断语库 789→1000+**（表外神煞旧死规则已于 2026-08-19 清理（`cc5e405`），扩充前无需再清）
