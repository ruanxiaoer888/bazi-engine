...
**当前状态（2026-08-28，v1.3.7）**：
- ✅ **v1.3.7 已发布**：CITY_LON 城市经度表扩充至 **724 城**（71→345 地级市全覆盖→+379 县级市，义乌/昆山/常熟等），匹配逻辑优化（最长名称优先 + 填"义乌"命中"义乌市"），Node 双端兼容修复，13 套回归全绿 + CI 合规门禁；**已推送 + tag v1.3.7 + GitHub Release 创建**
- ✅ 功能全部完成（132 项任务清零）、真人验收通过、**双平台上架**（SkillHub + ClawHub）、断语库 **1000 条**、**本仓库无阻塞**
- 🔜 下一步优先 **bazi-app 工作区验证交接**：切到 bazi-app 工作区新会话执行验证（黄冈/义乌/常熟/喀什校正 + 全流程回归，注意先清 localStorage 的 benchu.ctx.v2 防旧结果恢复）；之后是 bazi-app C 端真实闭环收尾（速贴块见本地归档）
**技术约定（详见 §九 项目速查）**：
...
| 发布材料 | ~~`docs/history/发布物料.md` + `docs/history/SkillHub-Submission-Kit.md`~~（2026-08-24 移出至 `E:\\michael\\Courses\\bazi-engine-internal`） | 卖点/图标提示词/示例对话/合规声明 + 提交包模板 |
...
### 当前状态（2026-08-28 快照）
- **2026-08-28 CITY_LON 城市经度表扩充至 724 城**（commit `a146409` + `2640495`）：71→345（地级市全覆盖）→724（+379 县级市，义乌/昆山/常熟/浏阳/慈溪/滕州/神木等）；匹配逻辑优化为**最长名称优先**（宜昌市宜都市→取宜都）+ **去"市"后缀兼容**（填"义乌"命中"义乌市"）；compliance 版已同步 bazi-app/web/engine.dist.js?v=5（C 端真太阳时校正生效）
- **2026-08-28 compliance_soften.py 修正**：`桃花` 移出 SENSITIVE（引擎神煞内部 key，全局替换破坏匹配；敏感组合词烂桃花/桃花正缘已有规则）；test_engine 新增 8 条 CITY_LON 县级市断言（36 PASS）
- **本仓库无阻塞**：132 项任务清零 / 真人验收通过 / 双平台已上架 / **断语库 1000 条**（路线图达成）/ **v1.3.6**（三方审计 6 批次全量修复）/ 13 套回归全绿 + **CI 门禁真实生效**；dist MD5 `1A4722FA7B0974EB4F5CFA53C71AA9C3`（LF 口径，README 优化未动引擎）；HEAD=`6497e7d`（2026-08-24 历史重写后的最新提交，旧 hash 已全部失效见顶部⚠️），git tag `v1.3.0`/`v1.3.6`（已重定向），工作树干净
- **在途事项（等待 Michael 操作，非本仓库代码工作）**：bazi-app C 端真实闭环剩 2 个外部阻塞（服务器四件套更新 + 虎皮椒真实支付接入——**微信渠道已开通，支付宝未开通**）；**速贴块已于 2026-08-19 交付**（本地归档 `E:\\michael\\Courses\\bazi-engine-internal\BAZI-APP-HANDOFF-v1.3.0.md` 顶部，2026-08-24 自 docs/ 移出），Michael 复制粘贴给 bazi-app 对话即可启动，执行到 ③④ 步时向对话提供 SSH 凭据 + 虎皮椒微信 appid/appsecret
...
### ✅ 2026-08-28 CITY_LON 城市经度表扩充（commit `a146409` + `2640495`，v1.3.7）
> 背景：bazi-app（C 端）真太阳时经度校正依赖引擎 CITY_LON 表，原仅 71 城，黄冈等大量地级市不在表内；B 端 UI 与 C 端共享同一引擎（唯一真源 `tools/build_ui.py` 内嵌 TPL）。
> 改动：
> - **CITY_LON 71→345**：补全全部地级市（含自治州/盟/地区），黄冈 114.9° 等
> - **CITY_LON 345→724**：+379 县级市（397 名单中跳过 18 个与自治州/地区首府重名者）。数据源：modood 行政区划名单 + GeoNames 坐标 + 县改市旧址经度（54 个）+ 常识补录 4 个兵团新市（盘州/双河/新星/白杨），误差 <0.3°≈1.2 分钟，远低于时辰边界 ±10 分钟敏感度
> - **匹配逻辑优化**：最长名称优先（`宜昌市宜都市`→取「宜都」而非「宜昌」）+ 县级市键去「市」后缀兼容（填「义乌」命中「义乌市」）
> - **compliance_soften.py 修正**：`桃花` 移出 SENSITIVE（引擎神煞内部 key：`{name:'桃花'}`/`case '桃花:'`/`s.name==='桃花'`，全局替换破坏匹配；敏感组合表述「烂桃花/桃花正缘」规则保留）
> - **test_engine.js 补 8 条断言**：724 城数量 + 关键县级市值 + 最长匹配/去市匹配行为（28→36 PASS）
> 验证：regions=7 构建通过；compliance --check 无敏感词残留；C 端实测黄冈（-20 分）/义乌（+0 分）/常熟（+3 分）/浏阳（-25 分）/喀什（-176 分）全部校正生效；13 套回归全绿
...
- **✅ 已发布完成**（2026-08-20）：用 `clawhub skill publish` CLI 重新发布 v1.3.6 成功（`clawhub login` 后直接 publish，安全检查约 2-3 分钟转公开，latest 已更新；注意 CLI 非 TTY 下退出码 1 是进度动画假象，用 `--json` 确认 `ok:true` 即可）；版本 v0.1.0 → **v1.3.6** ✅；页面描述已含双许可声明 ✅；**License 仍 MIT-0（平台强制，改不了）**
...
- **ClawHub 发布成功**（21:04）：clawhub.ai/skills 公开可见，一键安装 `npx clawhub@latest install bazi-engine`；**2026-08-17 SkillSpector 扫描通过转 Published**（Productivity 分类，v1.2.1 GitHub 自动同步版）（注：Import from GitHub 为一次性导入，非持续同步，见坑 #49）
...
- **对外文档**：`README.md`（项目门面：功能/快速开始/示例对话/免责声明）；内部发布材料（卖点/图标提示词/渠道建议）已移出至 `E:\\michael\\Courses\\bazi-engine-internal\发布物料.md`
...
### 交付文档
- ~~`发布物料.md`~~ — 6 大卖点 / 图标提示词 / 3 个示例对话 / 合规声明 / 渠道建议（2026-08-24 移出至 `E:\\michael\\Courses\\bazi-engine-internal`，含竞对分析/SkillHub-Submission-Kit）
...
- 发布文档：`docs/history/SkillHub发布最终指引.md`（填表指引）+ `docs/history/验收与截图清单_3案例.md`（回归验收模板）；提交母版 ~~`SkillHub-Submission-Kit.md`~~ 已移出至 `E:\\michael\\Courses\\bazi-engine-internal`
- **引擎变更记录：`docs/ENGINE-CHANGES.md`**（权威记录：每次引擎变更 bump 版本 + 记 dist MD5，供 bazi-app 对话核对；2026-08-17 建立，首条 v1.2.1）
- 历史文档：`docs/history/`（第七轮验收 / SkillHub发布最终指引 / 验收清单，历史快照；~~发布物料 / 竞对分析~~ 2026-08-24 移出至 `E:\\michael\\Courses\\bazi-engine-internal`）
- 截图素材：
...
- **bazi-app**（C 端「本初」，原「算了么」因品牌冲突已改名）：`github.com/ruanxiaoer888/bazi-app`（Private）
  ...
  - 状态：报告功能（25+ 板块）+ 海报分享 + 支付链路（虎皮椒 mock）+ 兑换码解锁均已完成；**待部署上线**（等 SSH+DNS），阻塞在服务器四件套更新 + 虎皮椒真实支付接入（**微信渠道已开通**，支付宝未；`xunhu.channel` 默认 `wechat`）。**闭环启动指令已交付**：本地归档 `E:\\michael\\Courses\\bazi-engine-internal\BAZI-APP-HANDOFF-v1.3.0.md`（顶部速贴版，2026-08-24 自 docs/ 移出，直接粘贴给 bazi-app 对话）
