...
rules = json.load(open(os.path.join(ROOT, 'knowledge-base', '04-断语库', '断语库.json'), encoding='utf-8'))['rules']
...
<div class="wrap">
  ...
  <div class="card form-card" id="formCard">
    ...
    <div class="row">
      <div><label>出生城市（用于真太阳时校正）</label><input id="birthplace" placeholder="如：湖北省黄冈市（县级市请填到县）"></div>
  ...
  <div id="result" class="hidden">
    ...
    <div class="card">
      ...
      <button class="ghost" onclick="runLiu()">分析该流年 →</button>
    ...
    <div class="card" id="liuMonthCard">
      ...
      <div class="row">
        ...
        <div style="display:flex;align-items:flex-end;flex-wrap:wrap"><button class="ghost" onclick="runLiuYue()" style="margin-top:0;white-space:normal;max-width:100%">展开十二流月 →</button></div>
      </div>
    ...
    <div class="card" id="liuDayCard">
      ...
      <div class="row">
        ...
        <div style="display:flex;align-items:flex-end;flex-wrap:wrap"><button class="ghost" onclick="runLiuDay()" style="margin-top:0;white-space:normal;max-width:100%">展开逐日运势 →</button></div>
      </div>
    ...
    <div class="card" id="liuQinCard">
      ...
      <div style="margin-top:10px;max-width:100%"><button class="ghost" onclick="runLiuQin()" style="margin-top:0;white-space:normal;max-width:100%">展开六亲详解 →</button></div>
          ...
          <div>
            ...
            <div class="time-group">
              ...
              <select id="hShichenA" class="hidden">
                <option value="">选时辰</option>
                <option value="子">子时</option><option value="丑">丑时</option><option value="寅">寅时</option><option value="卯">卯时</option>
                <option value="辰">辰时</option><option value="巳">巳时</option><option value="午">午时</option><option value="未">未时</option>
                <option value="申">申时</option><option value="酉">酉时</option><option value="戌">戌时</option><option value="亥">亥时</option>
              </select>
            </div>
          </div>
        </div>
        <div><label>出生城市</label><input id="hPlaceA" placeholder="如：湖北省黄冈市"></div>
      </div>
      <div>
...
function selYear(id){ const a=[]; for(let i=2100;i>=1895;i--) a.push({v:i,l:i+'年'}); selFill(document.getElementById(id), a, '年份'); }
...
function drawWxBar(ctx){
  const w=680,h=240,pad=20,barH=32,gap=10;
  const wx=['木','火','土','金','水'];
...
html = TPL.replace('__JIEQI__', json.dumps(jieqi, ensure_ascii=False)).replace('__RULES__', json.dumps(rules, ensure_ascii=False))
